...
OraSchool.controller('teachersController', function(dataFactory,$rootScope,$scope,$sce) {
    ...
    $scope.subject_array = {};
    $scope.subject_ids = {};
    $scope.filter_teachers_input_class = '0';
    $scope.filter_teachers_input_subject = '0';
    ...
    $scope.getCurrentSubjects = function(){
      $scope.subject_ids = $scope.classes.find(item => item.id === parseInt($scope.filter_teachers_input_class)).classSubjects;
    }

    $scope.filterTeachers1 = function(pageNumber) {
    		showHideLoad();
    		var input_class = $scope.filter_teachers_input_class;
    		var input_subject = $scope.filter_teachers_input_subject;

    		if($scope.filter_teachers_input_class != null && $scope.filter_teachers_input_class != '0') {
    			input_class = [$scope.filter_teachers_input_class];
    		} else {
    			input_class = '';
    		}
    		if($scope.filter_teachers_input_subject != null && $scope.filter_teachers_input_subject != '0') {
    			input_subject = [$scope.filter_teachers_input_subject];
    		} else {
    			input_subject = '';
    		}
    		if(
    			($scope.filter_teachers_input_class != null && $scope.filter_teachers_input_class != '0')
    			||
    			($scope.filter_teachers_input_subject != null && $scope.filter_teachers_input_subject != '0')
    		) {
    			send_data = {
	    			searchInput: {
	    				'class': input_class,
        ...
        dataFactory.httpRequest('index.php/teachers/listAll/'+pageNumber, 'GET', send_data).then(function(data) {
            $scope.teachers = data.teachers;
            $scope.totalItems = data.totalItems;
            showHideLoad(true);
        });
