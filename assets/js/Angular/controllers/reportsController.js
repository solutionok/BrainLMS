...
OraSchool.controller('reportsController', function(dataFactory,$rootScope,$scope,$http,$sce) {
    ...
    $scope.form.payment_mode = 'All';
    $scope.form.payment_status = '1';
    ...
    $scope.checkChangeClassFilter = function() {
			var class_id = $scope.form.classId
			$scope.form.sectionId = 0;
			if(class_id > 0) {
    ...
    $scope.fetchPaymentMethods = function() {
    	showHideLoad();
    	dataFactory.httpRequest('index.php/invoices/fetch-payment-methods').then(function(data) {
    ...
    $scope.getPayments = function(){
        showHideLoad();
        dataFactory.httpRequest('index.php/reports','POST',{},{
        	'stats': 'payments',
        	'data': $scope.form
        }).then(function(data) {
          if(data){
              $scope.payments = data.payments;
              $scope.invoicesTargetHashedIds = data.payment_hashed_ids;
    ...
    $scope.marksheetGenerationPrepare = function(){
        ...
        dataFactory.httpRequest('index.php/reports','POST',{},{'stats':'marksheetGenerationPrepare','data':$scope.form}).then(function(data) {
            if(data){
                ...
                $scope.form.classId = 1; // init
                $scope.checkChangeClassFilter(); // init
    ...
    $scope.changeView = function(view){
        ...
        $scope.views.donation_transactions = false;
        $scope.views[view] = true;

        if($scope.views.paymentsReports) {
			    $scope.fetchClasses();
			    $scope.fetchPaymentMethods();
		    }
    }
});